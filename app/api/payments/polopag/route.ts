...
import { upsertPoloPagPixConfig } from "lib/payments";
...
export async function PUT(request: Request) {
  try {
    ...
    const sanitizedApiKey = typeof apiKey === "string" ? apiKey : "";
    ...
    const config = await upsertPoloPagPixConfig({
      userId: user.id,
    ...
    console.error("Failed to update PoloPag config", error);
    return NextResponse.json(
