...
export async function GET(request: NextRequest) {
  try {
    ...
    const limit = Number.isFinite(parsedLimit)
      ...
      : 2000;
    const links = await listAffiliateMlLinksForUser(user.id, { limit });
    ...
    const affiliateUrl =
      typeof payload.affiliateUrl === "string" && payload.affiliateUrl.trim()
        ...
        : typeof payload.url === "string"
          ? payload.url
          : "";
    const link = await upsertAffiliateMlLinkForUser(user.id, {
      affiliateUrl,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const removed = await deleteAffiliateMlLinksForUser(user.id, {
      all,
      itemIds,
    });
