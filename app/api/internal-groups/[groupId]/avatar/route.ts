...
import {
  InternalGroupError,
  getInternalGroupAvatarAccess,
  updateInternalGroupAvatar,
...
export async function GET(_request: Request, context: Context) {
  try {
    ...
    const avatar = await getInternalGroupAvatarAccess(groupId, user.id);
...
const failure = (error: unknown) => {
  ...
  console.error("[internal-groups] avatar failed", error);
  return NextResponse.json({ message: "Não foi possível atualizar a foto." }, { status: 500 });
};
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const stored = await saveUploadedFile(file, `internal-groups/${groupId}/avatar`, {
      image: { maxWidth: 1024, maxHeight: 1024, fit: "cover", format: "webp", quality: 88 },
    });
    const result = await updateInternalGroupAvatar(groupId, user.id, stored);
...
export async function DELETE(_request: Request, context: Context) {
  try {
    ...
    if (!user) return NextResponse.json({ message: "Não autenticado." }, { status: 401 });
    const groupId = Number((await context.params).groupId);
    const result = await updateInternalGroupAvatar(groupId, user.id, null);
    if (result.previousAvatarPath) {
      await deleteUploadedFile(result.previousAvatarPath).catch(() => {});
    }
    emitInternalGroupEvent({ groupId, actorUserId: user.id, type: "group.updated" });
