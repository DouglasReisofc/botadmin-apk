...
export async function POST(
  ...
  try {
    ...
    const mediaType = typeof rawMediaType === "string"
      ? rawMediaType.trim().toLowerCase()
      : "document";
    if (!ALLOWED_MEDIA_TYPES.has(mediaType as any)) {
    ...
    const folder = `bot-groups/${groupId}/auto-responses`;
    ...
    console.error("Failed to upload autoresponse media", error);
    return NextResponse.json(
