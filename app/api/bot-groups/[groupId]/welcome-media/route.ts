...
export async function POST(
  ...
  try {
    ...
    const previousPath = currentSettings.welcomeConfig.mediaPath ?? null;

    const storedPath = await saveUploadedFile(file, `bot-groups/${groupId}/welcome`, {
      fixedFileName: "welcome-media",
      convertToWebp: false,
    });
    ...
    return NextResponse.json({
      message: "Mídia de boas-vindas atualizada com sucesso.",
      settings,
    });
  } catch (error) {
    console.error("Failed to upload welcome media", error);
    return NextResponse.json(
      { message: "Não foi possível enviar a mídia de boas-vindas." },
      { status: 500 },
    );
  }
}
