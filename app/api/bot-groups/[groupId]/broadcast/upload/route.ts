...
const resolveFolder = (groupId: number) => `bot-groups/${groupId}/broadcast`;
...
export async function POST(
  ...
  try {
    ...
    const folder = resolveFolder(groupId);
...
export async function DELETE(
  ...
  try {
    ...
    if (!path) {
      return NextResponse.json({ message: "Informe o caminho do arquivo." }, { status: 400 });
    }
    await deleteUploadedFile(path).catch(() => {});
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Failed to delete broadcast media", error);
    return NextResponse.json({ message: "Não foi possível excluir o arquivo." }, { status: 500 });
  }
}
