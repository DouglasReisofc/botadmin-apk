...
import { ytSearch as modernYtSearch, ytPlayMp3 } from 'lib/apis/yt';
...
export const GET = withUserApiAuth(async (req: NextRequest) => {
  try {
    ...
    // 1) Canonicaliza a URL/termo para corrigir case do ID
    ...
    // Aceita apenas YouTube para este endpoint; evita tratar links de mídia do WhatsApp, etc.
    ...
    try {
      const direct = await ytPlayMp3(canonical, { skipInternalEndpoint: true });
      const payload = {
        id: direct.id,
        title: direct.title,
        ...
        format: direct.format || 'audio/mpeg',
      };
    ...
    // 2) Garante cookies atualizados do YouTube antes de chamar o Python
    ...
    const bundle = infoJson?.audio_info || {};
    const payload = {
      id,
    ...
    // Se falhou, tenta forçar atualização de cookies e avisa erro ao cliente
    try {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 15000);
