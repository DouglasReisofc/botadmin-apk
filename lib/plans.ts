...
const mapPlanRow = (row: SubscriptionPlanRow): SubscriptionPlan => ({
  ...
  addonInstancePrice: roundToTwoDecimals(Number.parseFloat(row.addon_instance_price ?? "0")),
  addonGroupPrice: roundToTwoDecimals(Number.parseFloat(row.addon_group_price ?? "0")),
  groupLimit: Math.max(0, Number(row.group_limit ?? row.category_limit ?? 0)),
  instanceLimit: Math.max(0, Number(row.instance_limit ?? 0)),
  allowFlows: row.allow_flows === undefined ? true : row.allow_flows === 1,
  storageQuotaGb: roundToTwoDecimals(Number.parseFloat(row.storage_quota_gb ?? "0")),
