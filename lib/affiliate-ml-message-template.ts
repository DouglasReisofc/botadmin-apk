...
const DEFAULT_PROVIDER_TITLE = "*_Mercado Livre_*";
...
export type AffiliateMlMessageTemplateItem = {
  key: string;
  label: string;
  hint: string;
  enabled: boolean;
...
export type AffiliateMlMessageTemplateSummary = {
...
type AffiliateMlMessageTemplateRow = RowDataPacket & {
  id: number;
  user_id: number;
  provider: string;
...
const buildDefaultItems = (): AffiliateMlMessageTemplateItem[] => {
...
const normalizeStoredItems = (input: unknown): AffiliateMlMessageTemplateItem[] => {
...
const fetchTemplateRow = async (userId: number): Promise<AffiliateMlMessageTemplateRow | null> => {
  await ensureTemplateTable();
  const db = getDb();
  const [rows] = await db.query<AffiliateMlMessageTemplateRow[]>(
    `
      SELECT *
      FROM ${TABLE_NAME}
...
const mapSummary = (row: AffiliateMlMessageTemplateRow | null): AffiliateMlMessageTemplateSummary => {
  const parsed = (() => {
...
export const getAffiliateMlMessageTemplateForUser = async (
  userId: number,
...
export const saveAffiliateMlMessageTemplateForUser = async (
  userId: number,
  ...
  return await getAffiliateMlMessageTemplateForUser(userId);
};
...
export const renderAffiliateMlMessageTemplate = (
  items: AffiliateMlMessageTemplateItem[],
  context: Record<string, string>,
): string => {
...
export const getAffiliateMlTemplateTokensHelp = (): Array<{ token: string; description: string }> => {
  return [
    ...
    { token: "{{item_id}}", description: "ID do item no Mercado Livre." },
  ];
};

export const getDefaultAffiliateMlMessageTemplate = (): AffiliateMlMessageTemplateSummary => ({
  provider: "mercadolivre",
