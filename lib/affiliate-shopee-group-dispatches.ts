...
type AffiliateShopeeGroupDispatchRow = RowDataPacket & {
  id: number;
  user_id: number;
  provider: string;
  group_id: number;
...
export type AffiliateShopeeGroupDispatchSummary = {
  id: number;
  groupId: number;
  groupName: string;
...
export type AffiliateShopeeGroupDispatchWorkerEntry = {
  id: number;
  userId: number;
  groupId: number;
  groupName: string;
...
const mapSummary = (row: AffiliateShopeeGroupDispatchRow): AffiliateShopeeGroupDispatchSummary => {
  const groupName =
...
const ensureAffiliateShopeeGroupDispatchesTable = async () =>
...
const getDispatchById = async (
  userId: number,
  dispatchId: number,
): Promise<AffiliateShopeeGroupDispatchSummary | null> => {
  await ensureAffiliateShopeeGroupDispatchesTable();
  const db = getDb();
  const [rows] = await db.query<AffiliateShopeeGroupDispatchRow[]>(
    `
      SELECT
        d.*,
...
export const listAffiliateShopeeGroupDispatchesForUser = async (
  userId: number,
): Promise<AffiliateShopeeGroupDispatchSummary[]> => {
  await ensureAffiliateShopeeGroupDispatchesTable();
  const db = getDb();
  const [rows] = await db.query<AffiliateShopeeGroupDispatchRow[]>(
    `
...
export const upsertAffiliateShopeeGroupDispatchForUser = async (
  userId: number,
  payload: {
    groupId?: number;
    instanceId?: number;
...
export const updateAffiliateShopeeGroupDispatchForUser = async (
  userId: number,
  dispatchId: number,
  payload: {
    enabled?: boolean;
...
export const deleteAffiliateShopeeGroupDispatchForUser = async (
  userId: number,
  dispatchId: number,
): Promise<void> => {
  await ensureAffiliateShopeeGroupDispatchesTable();
...
export const listEnabledAffiliateShopeeGroupDispatchesForRun = async (
  limit = 25,
): Promise<AffiliateShopeeGroupDispatchWorkerEntry[]> => {
  await ensureAffiliateShopeeGroupDispatchesTable();
  ...
  const entries: AffiliateShopeeGroupDispatchWorkerEntry[] = [];
...
export const markAffiliateShopeeGroupDispatchSuccess = async (params: {
  userId: number;
  dispatchId: number;
...
export const markAffiliateShopeeGroupDispatchError = async (params: {
  userId: number;
  dispatchId: number;
  error: unknown;
}): Promise<void> => {
