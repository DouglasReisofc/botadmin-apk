...
type AffiliateMlLinkRow = RowDataPacket & {
  id: number;
  user_id: number;
  provider: string;
  item_id: string;
...
export type AffiliateMlLinkSummary = {
  id: number;
  itemId: string;
  affiliateUrl: string;
...
const mapLinkRow = (row: AffiliateMlLinkRow): AffiliateMlLinkSummary => ({
  id: Number(row.id),
...
const ensureAffiliateMlLinkTable = async () =>
  runEnsure("affiliate-ml-links-table", async () => {
...
const resolveProductFromLink = async (
  ...
  const searchResult = await searchMercadoLivre(affiliateUrl, { userId, limit: 1 });
  const products = Array.isArray(searchResult.produtos) ? searchResult.produtos : [];
  const product = (products[0] ?? null) as MercadoLivreProduct | null;
  const rawId = product?.id ? String(product.id) : String(searchResult.consulta?.itemId || "");
  const itemId = normalizeItemId(rawId);
  if (!itemId) {
    throw new Error(
      "Não foi possível identificar o item desse link de afiliado. Use o link direto do anúncio no Mercado Livre.",
    );
  }
  return {
    itemId,
...
const loadAffiliateMlLinkByItemId = async (
  db: ReturnType<typeof getDb>,
  userId: number,
  itemId: string,
): Promise<AffiliateMlLinkSummary> => {
  const [rows] = await db.query<AffiliateMlLinkRow[]>(
    `
      SELECT *
      FROM ${TABLE_NAME}
...
export const listAffiliateMlLinksForUser = async (
  userId: number,
  options: { limit?: number } = {},
): Promise<AffiliateMlLinkSummary[]> => {
  await ensureAffiliateMlLinkTable();
  const db = getDb();
  ...
  const byItemId = new Map<string, AffiliateMlLinkSummary>();
...
export const upsertAffiliateMlLinkForUser = async (
  userId: number,
  ...
  await db.query<ResultSetHeader>(
    `
      INSERT INTO ${TABLE_NAME} (
        user_id, provider, item_id, affiliate_url, category_id, note, coupon_code, coupon_details, title, product_url, image_url, price_amount, price_formatted, currency_id, is_available, is_active, last_used_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
  ...
  return loadAffiliateMlLinkByItemId(db, userId, resolved.itemId);
};

export const upsertAffiliateMlLinkDirectForUser = async (
  userId: number,
  payload: {
    itemId: string;
    affiliateUrl: string;
  ...
  await dedupeAffiliateUrlRows(db, userId);
  return loadAffiliateMlLinkByItemId(db, userId, itemId);
};

export const upsertAffiliateMlLinksBatchForUser = async (
  userId: number,
  entries: Array<{
    itemId?: string | null;
  ...
  const links: AffiliateMlLinkSummary[] = [];
  const errors: string[] = [];
  ...
  for (let index = 0; index < normalizedEntries.length; index += chunkSize) {
    ...
    const chunkResults = await Promise.allSettled(
      chunk.map((entry) =>
        upsertAffiliateMlLinkDirectForUser(userId, {
          itemId: entry.itemId,
...
export const deleteAffiliateMlLinkForUser = async (
  userId: number,
  itemIdRaw: string,
): Promise<void> => {
  await ensureAffiliateMlLinkTable();
...
export const deleteAffiliateMlLinksForUser = async (
  userId: number,
  options: {
    itemIds?: string[];
    all?: boolean;
...
export const updateAffiliateMlLinkForUser = async (
  userId: number,
  itemIdRaw: string,
  payload: {
    affiliateUrl?: string | null;
  ...
  const db = getDb();
  const [existingRows] = await db.query<AffiliateMlLinkRow[]>(
    `
      SELECT *
      FROM ${TABLE_NAME}
      WHERE user_id = ? AND provider = ? AND item_id = ?
...
export const resolveAffiliateMlLinkForUserByItemId = async (
  userId: number,
  itemIdRaw: string,
): Promise<AffiliateMlLinkSummary | null> => {
  await ensureAffiliateMlLinkTable();
  const itemId = normalizeItemId(itemIdRaw);
  if (!itemId) {
    return null;
...
export const markAffiliateMlLinkUsage = async (
  userId: number,
  itemIdRaw: string,
): Promise<void> => {
  await ensureAffiliateMlLinkTable();
...
export const updateAffiliateMlLinkCategoryForUser = async (
  userId: number,
  itemIdRaw: string,
...
const resolveLatestAvailability = (product: MercadoLivreProduct | null): boolean | null => {
...
const fetchLatestProductByItemId = async (
  ...
  try {
    const search = await searchMercadoLivre(itemId, { userId, limit: 1 });
  ...
  try {
    const search = await searchMercadoLivre(itemId, { limit: 1 });
    if (Array.isArray(search.produtos) && search.produtos.length > 0) {
      return search.produtos[0] ?? null;
    }
  } catch {
...
export const refreshAffiliateMlLinksSnapshotForUser = async (
  userId: number,
