...
export type AffiliateShopeeAutoSyncSummary = {
  provider: "shopee";
  enabled: boolean;
...
export type AffiliateShopeeAutoSyncWorkerEntry = {
  userId: number;
  enabled: boolean;
  refreshExisting: boolean;
  discoverNew: boolean;
...
const mapSummary = (row: AutoSyncRow | null): AffiliateShopeeAutoSyncSummary => ({
  provider: "shopee",
...
export const getAffiliateShopeeAutoSyncConfigForUser = async (
  userId: number,
): Promise<AffiliateShopeeAutoSyncSummary> => {
  const row = await getRowByUserId(userId);
  return mapSummary(row);
};

export const upsertAffiliateShopeeAutoSyncConfigForUser = async (
  userId: number,
  payload: {
    enabled?: unknown;
  ...
  await ensureAutoSyncTable();
  const current = await getAffiliateShopeeAutoSyncConfigForUser(userId);
  const nextEnabled =
  ...
  return await getAffiliateShopeeAutoSyncConfigForUser(userId);
};

export const listAffiliateShopeeAutoSyncConfigsForRun = async (
  limit = 20,
): Promise<AffiliateShopeeAutoSyncWorkerEntry[]> => {
  await ensureAutoSyncTable();
  const db = getDb();
  ...
  const entries: AffiliateShopeeAutoSyncWorkerEntry[] = [];
...
export const markAffiliateShopeeAutoSyncSuccess = async (params: {
  userId: number;
}): Promise<void> => {
  await ensureAutoSyncTable();
  const db = getDb();
...
export const markAffiliateShopeeAutoSyncError = async (params: {
  userId: number;
  error: unknown;
}): Promise<void> => {
  await ensureAutoSyncTable();
