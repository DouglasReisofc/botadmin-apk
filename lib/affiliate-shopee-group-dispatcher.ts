...
import {
  listEnabledAffiliateShopeeGroupDispatchesForRun,
  markAffiliateShopeeGroupDispatchError,
  markAffiliateShopeeGroupDispatchSuccess,
  type AffiliateShopeeGroupDispatchWorkerEntry,
} from "lib/affiliate-shopee-group-dispatches";
import {
  listAffiliateShopeeLinksForUser,
  markAffiliateShopeeLinkUsage,
  updateAffiliateShopeeLinkCategoryForUser,
  type AffiliateShopeeLinkSummary,
} from "lib/affiliate-shopee-links";
import {
  getAffiliateMlDispatchContextSnapshot as getAffiliateShopeeDispatchContextSnapshot,
  recordAffiliateMlDispatchForContext as recordAffiliateShopeeDispatchForContext,
} from "lib/affiliate-ml-dispatch-history";
import {
  getAffiliateShopeeMessageTemplateForUser,
  renderAffiliateShopeeMessageTemplate,
  type AffiliateShopeeMessageTemplateSummary,
} from "lib/affiliate-shopee-message-template";
import { searchShopeeAffiliate, type ShopeeAffiliateProduct } from "lib/apis/shopee-affiliate";
import { sendInteractiveButtons, sendMediaMessage, sendTextMessage } from "lib/wuzapi";

const DISPATCH_INTERVAL_MS = Math.max(15_000, Number(process.env.AFFILIATE_SHOPEE_GROUP_DISPATCH_INTERVAL_MS ?? 45_000));
const DISPATCH_BATCH_SIZE = Math.max(1, Number(process.env.AFFILIATE_SHOPEE_GROUP_DISPATCH_BATCH ?? 12));
const TEMPLATE_CACHE_TTL_MS = 120_000;
const CATEGORY_HISTORY_LIMIT = 4;
const HISTORY_CAMPAIGN_ID = 0;
const HISTORY_CONTENT_ID = "affiliate_shopee_group_dispatch";
const AFFILIATE_SHOPEE_PROVIDER_TITLE = "*_Shopee_*";
const AFFILIATE_SHOPEE_DISPATCH_FOOTER = "Clique aqui para acessar 👇";
const AFFILIATE_SHOPEE_DISPATCH_BUTTON_LABEL = "Acessar oferta 🔥";
...
const templateCache = new Map<
  ...
  {
    expiresAt: number;
    promise: Promise<AffiliateShopeeMessageTemplateSummary>;
  }
>();

const runtime = globalThis as typeof globalThis & {
  __affiliateShopeeGroupDispatcherStarted?: boolean;
};

let dispatcherStarted = runtime.__affiliateShopeeGroupDispatcherStarted ?? false;
let dispatchCycleRunning = false;

const log = (message: string, extra?: Record<string, unknown>) => {
  console.log(`[AffiliateShopeeGroupDispatcher] ${message}`, extra ?? {});
};
...
const normalizeTargetKey = (entry: AffiliateShopeeGroupDispatchWorkerEntry): string => {
  return `dispatch:${entry.id}:group:${entry.groupId}`;
};

const computeDeterministicDelayFactor = (
  entry: AffiliateShopeeGroupDispatchWorkerEntry,
  lastSentAtMs: number,
): number => {
...
const getCachedMessageTemplate = async (userId: number): Promise<AffiliateShopeeMessageTemplateSummary> => {
  const now = Date.now();
  ...
  const promise = getAffiliateShopeeMessageTemplateForUser(userId);
...
const pickLeastRecentlyUsed = (
  ...
  delayMinutes: number,
): AffiliateShopeeLinkSummary | null => {
...
const toResolvedProduct = (link: AffiliateShopeeLinkSummary, product: ShopeeAffiliateProduct | null): ResolvedProduct => {
  const resolvedTitle =
  ...
  const ratingText =
    (typeof product?.shopee?.ratingStar === "string" && product.shopee.ratingStar.trim()
      ? product.shopee.ratingStar.trim()
      : typeof link.ratingStar === "string" && link.ratingStar.trim()
        ? link.ratingStar.trim()
        : "");
...
const resolveProductDetails = async (
  userId: number,
  link: AffiliateShopeeLinkSummary,
  ...
  try {
    const search = await searchShopeeAffiliate(itemId, {
      userId,
      limit: 1,
    });
...
const renderMessageBody = async (params: {
  ...
  templateItems?: AffiliateShopeeMessageTemplateSummary["items"] | null;
}): Promise<string> => {
  const fallbackLines: string[] = ["🛒 *_Shopee_*", `📦 *${params.product.title}*`];
  ...
  if (params.product.ratingText) {
    fallbackLines.push(`⭐ Avaliação: ${params.product.ratingText}`);
  }
  ...
  try {
    ...
    const rendered = renderAffiliateShopeeMessageTemplate(templateItems, {
      ...
      avaliacao: params.product.ratingText,
      rating: params.product.ratingText,
    ...
    const baseRendered = rendered || fallbackBody;
    if (params.product.ratingText && !baseRendered.includes(params.product.ratingText)) {
      return `${baseRendered}\n⭐ Avaliação: ${params.product.ratingText}`.trim();
    }
    return baseRendered;
  } catch {
    return fallbackBody;
...
const sendDispatchMessage = async (params: {
  ...
  const footerText = params.footerText.trim() || AFFILIATE_SHOPEE_DISPATCH_FOOTER;
  const buttonLabel = params.buttonLabel.trim() || AFFILIATE_SHOPEE_DISPATCH_BUTTON_LABEL;
  const providerTitle = params.providerTitle.trim() || AFFILIATE_SHOPEE_PROVIDER_TITLE;
  ...
  if (params.includeButtons) {
    try {
      await sendInteractiveButtons(params.client, {
        ...
        buttons: [
          {
            id: "affiliate_shopee_group_dispatch_open",
            text: buttonLabel,
            type: "cta_url",
            url: params.finalUrl,
          },
...
const processDispatchEntry = async (
  entry: AffiliateShopeeGroupDispatchWorkerEntry,
  linksByUserCache: Map<number, Promise<AffiliateShopeeLinkSummary[]>>,
): Promise<void> => {
  const targetKey = normalizeTargetKey(entry);
  const history = await getAffiliateShopeeDispatchContextSnapshot({
    userId: entry.userId,
  ...
  if (!linksPromise) {
    linksPromise = listAffiliateShopeeLinksForUser(entry.userId);
  ...
  const queryText = chosenLink.note?.trim() || "shopee";
  const template = await getCachedMessageTemplate(entry.userId).catch(() => null);
  const footerText = template?.footerText?.trim() || AFFILIATE_SHOPEE_DISPATCH_FOOTER;
  const buttonLabel = template?.buttonLabel?.trim() || AFFILIATE_SHOPEE_DISPATCH_BUTTON_LABEL;
  const providerTitle = template?.providerTitle?.trim() || AFFILIATE_SHOPEE_PROVIDER_TITLE;
  ...
  if (itemId && categoryId && !chosenLinkHadCategory) {
    await updateAffiliateShopeeLinkCategoryForUser(entry.userId, itemId, categoryId).catch(() => {
      // sem impacto no envio principal
    });
  }

  await recordAffiliateShopeeDispatchForContext({
    userId: entry.userId,
    campaignId: HISTORY_CAMPAIGN_ID,
    targetId: entry.id,
    targetKey,
  ...
  if (itemId) {
    await markAffiliateShopeeLinkUsage(entry.userId, itemId).catch(() => {
      // sem impacto no envio principal
    });
  }

  await markAffiliateShopeeGroupDispatchSuccess({
    userId: entry.userId,
    dispatchId: entry.id,
...
const runDispatchCycle = async (): Promise<void> => {
  ...
  try {
    const entries = await listEnabledAffiliateShopeeGroupDispatchesForRun(DISPATCH_BATCH_SIZE);
    if (!Array.isArray(entries) || entries.length === 0) {
      return;
    }

    const linksByUserCache = new Map<number, Promise<AffiliateShopeeLinkSummary[]>>();
    for (const entry of entries) {
      try {
        await processDispatchEntry(entry, linksByUserCache);
      } catch (error) {
        await markAffiliateShopeeGroupDispatchError({
          userId: entry.userId,
          dispatchId: entry.id,
          error,
        }).catch(() => {
...
export const startAffiliateShopeeGroupDispatcher = () => {
  if (dispatcherStarted) {
    return;
  }
  dispatcherStarted = true;
  runtime.__affiliateShopeeGroupDispatcherStarted = true;
