...
export function usePanelModules(userId: number | undefined, scope: "user" | "admin") {
  ...
  const toggle = async (id: string, next: boolean): Promise<boolean> => {
    if (busy.current || loading) return false;
    const moduleState = modules.find((item) => item.id === id);
    if (next && moduleState && !moduleState.canEnable) { setError(moduleState.reason || "Este módulo não pode ser ativado."); return false; }
    ...
    try {
      ...
      if (current === generation.current) {
        const received = Array.isArray(data.modules) ? data.modules as ModuleState[] : [];
        if (received.length) {
          setModules(received);
          setEnabled(received.filter((item) => item.enabled).map((item) => item.id));
        } else {
          setEnabled(previous => next ? [...new Set([...previous, id])] : previous.filter(item => item !== id));
          setModules(previous => previous.map((item) => item.id === id ? { ...item, enabled: next, pinned: next } : item));
        }
      }
      return true;
    } catch (cause) { if (current === generation.current) setError(cause instanceof Error ? cause.message : "Falha ao salvar módulo."); return false; }
...
type ModuleAuditEntry = { id: number; actorName: string; targetName: string; scope: string; module: string; action: string; createdAt: string };
function useModuleGovernance(active: boolean) {
  const [states, setStates] = useState<GlobalModuleState[]>([]);
  const [audit, setAudit] = useState<ModuleAuditEntry[]>([]);
  const [saving, setSaving] = useState<string | null>(null);
  const [error, setError] = useState("");
  const reload = useCallback(async () => {
    ...
    try {
      const [stateResponse, auditResponse] = await Promise.all([
        fetch("/api/admin/panel-modules", { credentials: "include", cache: "no-store" }),
        fetch("/api/admin/panel-modules/audit?limit=30", { credentials: "include", cache: "no-store" }),
      ]);
      const [stateData, auditData] = await Promise.all([stateResponse.json(), auditResponse.json()]);
      if (!stateResponse.ok) throw new Error(stateData.message || "Falha ao carregar governança.");
      if (!auditResponse.ok) throw new Error(auditData.message || "Falha ao carregar histórico.");
      setStates(Array.isArray(stateData.modules) ? stateData.modules : []);
  ...
  const save = async (id: string, change: { enabled?: boolean; lifecycle?: PanelModuleLifecycle; rolloutPercent?: number }) => {
    ...
    try { const response = await fetch("/api/admin/panel-modules", { method: "PATCH", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ module: id, ...change }) }); const data = await response.json(); if (!response.ok) throw new Error(data.message || "Falha ao salvar governança."); if (data.state) setStates((current) => current.map((item) => item.id === id ? data.state : item)); await reload(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Falha ao salvar governança."); }
    finally { setSaving(null); }
  };
  return { states, audit, saving, error, save, reload };
}
...
export function PanelModules({ items, state, onOpen, scope = "user" }: { items: CatalogItem[]; state: ReturnType<typeof usePanelModules>; onOpen: (id: string) => void; scope?: "user" | "admin" }) {
  ...
  return <section className="panel-modules"><header><div className="panel-modules-heading-icon"><LayoutGrid size={26} /></div><div><h1>Módulos</h1><p>Ative somente os recursos que sua operação precisa.</p></div></header>
    ...
    {state.loading ? <p role="status">Carregando módulos…</p> : filteredItems.length ? <div className="panel-modules-grid">{filteredItems.map(item => { const remote = state.modules.find((module) => module.id === item.id); const global = governance.states.find((module) => module.id === item.id); const active = remote?.enabled ?? state.enabled.includes(item.id); const available = remote?.canEnable !== false; const availability = remote?.availability || "available"; return <article key={item.id} className={`${active ? "is-enabled" : ""} ${available ? "" : "is-locked"}`}><div className="panel-module-art"><img src={item.image} alt="" loading="lazy" />{item.badge && <span className="panel-module-badge">{item.badge}</span>}</div><div className="panel-module-copy"><div className="panel-module-title-row"><h2>{item.label}</h2><span className={`panel-module-status status-${availability}`}>{availability === "available" ? (active ? "Ativo" : "Disponível") : availability === "plan_locked" ? "Plano" : availability === "maintenance" ? "Manutenção" : availability === "coming_soon" ? "Em breve" : "Dependência"}</span></div><p>{item.description}</p>{item.id === "broadcasts" && active && <small className="panel-module-reason">Desativar pausa todos os envios. Ao reativar, cada transmissão precisará ser ativada novamente.</small>}{!available && remote?.reason && <small className="panel-module-reason">{remote.reason}</small>}{scope === "admin" && global && <div className="panel-module-governance"><label><span>Disponibilidade global</span><input type="checkbox" checked={global.globalEnabled} disabled={governance.saving === item.id} onChange={(event) => void governance.save(item.id, { enabled: event.target.checked })} /></label><select value={global.lifecycle} disabled={governance.saving === item.id} onChange={(event) => void governance.save(item.id, { lifecycle: event.target.value as PanelModuleLifecycle })} aria-label={`Estado global de ${item.label}`}><option value="active">Ativo</option><option value="beta">Beta</option><option value="maintenance">Manutenção</option><option value="coming_soon">Em breve</option></select></div>}</div><footer><button type="button" className="panel-module-switch" role="switch" aria-checked={active} aria-label={`${active ? "Desativar" : "Ativar"} ${item.label}`} disabled={state.saving !== null || !available} onClick={() => { if (active && item.id === "broadcasts" && !window.confirm("Desativar Transmissão pausará todos os envios pendentes e em andamento. Depois, cada transmissão deverá ser ativada manualmente. Continuar?")) return; void state.toggle(item.id, !active).then((saved) => { if (saved && !active && item.id === "payments") onOpen(item.id); }); }}><span className="panel-module-track"><i /></span>{state.saving === item.id ? "Salvando…" : active ? "Ativado e no menu" : available ? "Ativar módulo" : "Indisponível"}</button>{active && available && <button className="panel-module-open" type="button" onClick={() => onOpen(item.id)} aria-label={`Abrir ${item.label}`}><ArrowUpRight size={18} />Abrir</button>}</footer></article>; })}</div> : <div className="panel-modules-empty">Nenhum módulo corresponde à busca.</div>}
    ...
    {scope === "admin" && governance.audit.length > 0 && <section className="panel-modules-audit" aria-label="Histórico dos módulos"><header><div><h2>Atividade recente</h2><p>Alterações de disponibilidade, ativação e organização.</p></div><button type="button" onClick={() => void governance.reload()}>Atualizar</button></header><div className="panel-modules-audit-list">{governance.audit.map((entry) => { const item = items.find((candidate) => candidate.id === entry.module); const action = entry.action === "module.enabled" ? "ativou" : entry.action === "module.disabled" ? "desativou" : entry.action === "module.layout" ? "organizou" : "alterou"; return <article key={entry.id}><span className="panel-modules-audit-dot" /><div><strong>{entry.actorName}</strong><p>{action} <b>{item?.label || entry.module}</b>{entry.targetName && entry.targetName !== entry.actorName ? ` para ${entry.targetName}` : ""}</p></div><time dateTime={entry.createdAt}>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(entry.createdAt))}</time></article>; })}</div></section>}
  </section>;
}
