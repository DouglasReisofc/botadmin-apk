...
type CatalogItem = { id: string; label: string; description: string; icon: typeof LayoutGrid; image: string; badge?: string };
export function PanelModules({ items, state, onOpen }: { items: CatalogItem[]; state: ReturnType<typeof usePanelModules>; onOpen: (id: string) => void }) {
  return <section className="panel-modules"><header><div className="panel-modules-heading-icon"><LayoutGrid size={26} /></div><div><span className="panel-modules-kicker">CATÁLOGO DO PAINEL</span><h1>Módulos</h1><p>Ative somente os recursos que sua operação precisa.</p></div></header>
