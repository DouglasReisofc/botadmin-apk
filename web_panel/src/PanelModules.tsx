...
import { PANEL_MODULES, getPanelModules, type PanelModuleAvailability } from "../../lib/panel-modules";
...
export function PanelModules({ items, state, onOpen }: { items: CatalogItem[]; state: ReturnType<typeof usePanelModules>; onOpen: (id: string) => void }) {
  ...
  return <section className="panel-modules"><header><div className="panel-modules-heading-icon"><LayoutGrid size={26} /></div><div><h1>Módulos</h1><p>Ative somente os recursos que sua operação precisa.</p></div></header>
    ...
    {state.loading ? <p role="status">Carregando módulos…</p> : filteredItems.length ? <div className="panel-modules-grid">{filteredItems.map(item => { const remote = state.modules.find((module) => module.id === item.id); const active = remote?.enabled ?? state.enabled.includes(item.id); const available = remote?.canEnable !== false; const availability = remote?.availability || "available"; return <article key={item.id} className={`${active ? "is-enabled" : ""} ${available ? "" : "is-locked"}`}><div className="panel-module-art"><img src={item.image} alt="" loading="lazy" />{item.badge && <span className="panel-module-badge">{item.badge}</span>}</div><div className="panel-module-copy"><div className="panel-module-title-row"><h2>{item.label}</h2><span className={`panel-module-status status-${availability}`}>{availability === "available" ? (active ? "Ativo" : "Disponível") : availability === "plan_locked" ? "Plano" : availability === "maintenance" ? "Manutenção" : availability === "coming_soon" ? "Em breve" : "Dependência"}</span></div><p>{item.description}</p>{!available && remote?.reason && <small className="panel-module-reason">{remote.reason}</small>}</div><footer><button type="button" className="panel-module-switch" role="switch" aria-checked={active} aria-label={`${active ? "Desativar" : "Ativar"} ${item.label}`} disabled={state.saving !== null || !available} onClick={() => void state.toggle(item.id, !active)}><span className="panel-module-track"><i /></span>{state.saving === item.id ? "Salvando…" : active ? "Ativado" : available ? "Ativar módulo" : "Indisponível"}</button>{active && available && <button className="panel-module-open" type="button" onClick={() => onOpen(item.id)} aria-label={`Abrir ${item.label}`}><ArrowUpRight size={18} />Abrir</button>}{available && <button type="button" className={`panel-module-pin ${remote?.pinned ? "active" : ""}`} onClick={() => void state.layout(item.id, { pinned: !remote?.pinned })} aria-label={remote?.pinned ? `Desafixar ${item.label}` : `Fixar ${item.label}`} title={remote?.pinned ? "Desafixar no menu" : "Fixar no menu"}>⌖</button>}</footer></article>; })}</div> : <div className="panel-modules-empty">Nenhum módulo corresponde à busca.</div>}
  </section>;
}
