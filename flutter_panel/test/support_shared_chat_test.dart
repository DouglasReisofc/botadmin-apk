...
class _SupportApi extends Fake implements BotAdminApiClient {
  ...
  final conversation = AdminSupportConversation.fromJson({
    ...
    'messages': [
      {'id': 1, 'senderRole': 'user', 'direction': 'outbound', 'text': 'Mensagem do usuário', 'timestamp': '2026-09-20T12:00:00Z'},
      {'id': 2, 'senderRole': 'admin', 'direction': 'inbound', 'text': 'Resposta do administrador', 'timestamp': '2026-09-20T12:01:00Z', 'deliveryState': 'read'},
    ],
  });
  @override
  Future<AdminSupportConversation> loadUserSupportConversation({String whatsappId = '__admin__'}) async {
    adminRequested = false;
    return conversation;
  }
  @override
  Future<AdminSupportConversation> loadAdminSupportConversation({required int userId, required String whatsappId}) async {
    adminRequested = true;
    return conversation;
  }
}
...
void main() {
  for (final admin in [false, true]) {
    testWidgets('support uses normal bubbles and composer as ${admin ? 'admin' : 'user'}', (tester) async {
      ...
      await tester.pumpWidget(ProviderScope(
        ...
        child: MaterialApp(home: Scaffold(body: UserSupportChatScreen(
          thread: snapshot.threads.single,
          adminEntry: admin ? AdminSupportThreadEntry(user: const AdminSupportUser(id: 1, name: 'Cliente'), thread: api.conversation.thread) : null,
        ))),
      ));
      ...
      final bubbles = tester.widgetList<ConversationMessageBubble>(find.byType(ConversationMessageBubble)).toList();
  ...
  test('support adapts authors, audio and genuine receipts to normal chat', () {
    final message = AdminSupportMessage.fromJson({
      'id': 9, 'direction': 'inbound', 'senderRole': 'admin',
      'timestamp': '2026-09-20T12:00:00Z', 'messageType': 'audio',
      'deliveryState': 'read',
      'media': {'mediaType': 'audio', 'mediaId': '42', 'mimeType': 'audio/mp4'},
    });
    final admin = message.toChatMessage(forAdmin: true, isAdminThread: true, incomingName: 'Cliente');
    final user = message.toChatMessage(forAdmin: false, isAdminThread: true, incomingName: 'Admin');
    ...
    expect(AdminSupportMessage.fromJson({'id': 10}).deliveryState, MessageDeliveryState.sent);
  });

  for (final width in [320.0, 390.0, 1280.0]) {
    testWidgets('normal chat composer works at $width without lottery', (tester) async {
      ...
      Widget composer() => ConversationComposer(
        controller: controller, mentionAll: false, buttonsEnabled: false,
        buttons: const [], botEnabled: false, showBotButton: false,
        showStoreButton: false, internalGroup: false,
        voiceRecording: false, voiceRecordingBusy: false,
        voiceDuration: Duration.zero, voiceViewOnce: false,
        onSend: () async { sent++; }, onAttach: () async { attachments++; },
        onEmoji: () { emojis++; }, onStore: () {}, showSweepstakeButton: false,
        onSweepstake: () {}, onBot: () {}, onVoiceStart: () async {},
        onVoiceStop: () async {}, onCancelVoice: () async {},
        onVoiceViewOnceChanged: null, onMentionAllChanged: null,
        onEditButtons: null, onClearButtons: null,
      );
      await tester.pumpWidget(ProviderScope(child: MaterialApp(home: Scaffold(
        body: Column(children: [const Spacer(), composer()]),
      ))));
      expect(tester.takeException(), isNull);
      expect(find.byTooltip('Segure para gravar · arraste para cancelar'), findsOneWidget);
      ...
      await tester.pumpWidget(MaterialApp(home: Scaffold(body: Column(children: [composer(), composer()]))));
      expect(tester.takeException(), isNull);
      await tester.pumpWidget(const SizedBox());
      controller.dispose();
    });
